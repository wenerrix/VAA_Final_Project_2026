...
customer_data <- robust_impute(customer_data) 
...
# 对合成的 Master Dataset 也补齐派生出来的数值列空值
master_dataset <- robust_impute(master_dataset)
...
ui <- dashboardPage(
  skin = "blue",
  dashboardHeader(title = span(class = "brand-title", "Fintech Customer"), titleWidth = 330),
  dashboardSidebar(
    width = 330,
    sidebarMenu(
      id = "nav_tabs",
      ...
      menuItem("Data Analysis", icon = icon("bar-chart-o"), startExpanded = FALSE, menuSubItem("Customer Profile", tabName = "analysis_profile"), menuSubItem("Transaction Behavior", tabName = "analysis_tx"), menuSubItem("CLV / Churn Analysis", tabName = "analysis_clv")),
      ...
      menuItem("Business Insights", icon = icon("lightbulb-o"), startExpanded = FALSE, menuSubItem("Key Findings", tabName = "biz_findings"), menuSubItem("Segment Actions", tabName = "biz_actions"), menuSubItem("CLV Driver Summary", tabName = "biz_drivers")),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "about_page", section_box("About This App", "blue", tags$p("This Shiny app follows the architecture we discussed earlier and organises the analysis into seven major sections: Dashboard, Data Analysis, Clustering, CLV Decision Tree, Business Insights, About, and Shared Data Backbone."), tags$ul(tags$li("Dashboard: KPI cards, customer snapshot, and segment snapshot."), tags$li("Data Analysis: profile analysis, transaction behaviour, and CLV/churn diagnostics."), tags$li("Clustering: K-means segmentation with PCA overview, profile heatmap, and cluster comparison."), tags$li("CLV Decision Tree: configurable rpart model for clv_segment classification."), tags$li("Business Insights: executive findings, segment-level action suggestions, and driver summaries."), tags$li("Shared Data Backbone: filtered customer data, transaction data, enriched master data, and downloads.")), tags$hr(), tags$p(paste("Customer records:", fmt_num(nrow(customer_data)))), tags$p(paste("Transaction records:", fmt_num(nrow(transactions)))), tags$p(paste("Transaction date range:", format(min_tx_date), "to", format(max_tx_date))), tags$p("The app expects customer_data.csv and transactions_data.csv to be placed in the same folder as app.R, unless you modify DATA_DIR in the configuration block.")))
    )
  )
)
