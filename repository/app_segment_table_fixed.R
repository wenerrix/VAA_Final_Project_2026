  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          column(12, section_box("Segment Summary Table", "blue", tableOutput("segment_summary_tbl")))
        )
      ),
      tabItem(
...
business_driver_tbl <- reactive({
  ...
  output$segment_summary_tbl <- renderTable({
    df <- filtered_customers() %>%
      dplyr::group_by(customer_segment) %>%
      dplyr::summarise(
        Customers = dplyr::n(),
        Average_CLV = safe_mean(customer_lifetime_value),
        Average_Churn = safe_mean(churn_probability),
        Average_Satisfaction = safe_mean(satisfaction_score),
        Average_Products = safe_mean(active_products),
        .groups = "drop"
      ) %>%
      dplyr::rename(Segment = customer_segment) %>%
      dplyr::mutate(
        Segment = as.character(Segment),
        Customers = fmt_num(Customers),
        Average_CLV = fmt_num_short(Average_CLV, 1),
        Average_Churn = fmt_pct(Average_Churn, 1),
        Average_Satisfaction = round(Average_Satisfaction, 2),
        Average_Products = round(Average_Products, 2)
      ) %>%
      as.data.frame(stringsAsFactors = FALSE)

    validate(
      need(nrow(df) > 0, "No customers available under the current filters.")
    )
